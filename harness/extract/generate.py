...
def render_components_css(
    ...
    brand_color = "var(--color-brand, #e4f222)"
    if design_system is not None and getattr(design_system, "colors", None):
        ...
        if brand_tok:
            brand_color = f"var({brand_tok.token_name}, {brand_tok.hex_value})"
    ...
    css_blocks = [
        ...
        "  color: var(--color-on-brand, #08090a);",
        ...
        "}",
        ".input-field:focus {",
        "  border-color: var(--color-brand, #e4f222);",
        "}",
        "",
        "/* Badge Components */",
        ".badge {",
