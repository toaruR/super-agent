...
- 設計とタスクの紐付けが塊単位で一目瞭然
- **論理的な追記モデル（イベントログ）を維持**: 過去イベントの改変・削除は行わず履歴を蓄積（物理的には 1 設計 1 行チャンク構造を保つため、一時ファイル経由のアトミックな全置換 `_rewrite()` で更新）。
- **パス正規化**: `design_file` および `task_file` のパスは常に `Path.resolve()` により物理フルパスへ正規化して記録・検索される。
- **チャンクマージ**: `append_event` は `design_file` の一致（`_same_path`）で同一チャンク（行）へイベントをマージする。`architect` 実行時点（`task_file=""`）のチャンクへ後から `plan`（`task_file="tasks.md"`）でイベントを追記すると、同一チャンクにマージされ、チャンクの `task_file` フィールドも自動的に更新される。

> **設計理由**: 従来は全タスクのイベントが単一の `events.jsonl` に `task_id` ごとに散在し、タスクファイルとの紐付けが不明だった。1塊 = 1設計とすることで、「台帳 → 元ファイル」の逆引きが構造的に保証される。

## 2. 台帳（ledger）仕様

**ファイル**: `harness/ledger/events.jsonl`（1塊 = 1行の JSONL。更新時は一時ファイル経由のアトミックな全置換 `_rewrite` で書き込み、改行終端）
...
- `task/<id>` を `--target`（既定はリポジトリの実際の主ブランチを自動検出、`detect_primary_branch()`。`harness/core/invoke.py`）へ `--no-ff` マージ。コンフリクトは `--abort` して検知。
...
- `--target` 省略時は `--design_file` から導出した `design/<stem>-<crc32>` ブランチが統合先になる（`design_branch_name()`、`harness/roles/scheduler.py`）。存在しなければ自動作成される。`--design_file` 未設定時のみ、リポジトリの実際の主ブランチを自動検出してフォールバック（`detect_primary_branch()`）。固定の統合先を使いたい場合は `--target` で明示指定する。
