...
- 目的: `ARCHITECTURE.md` §7 の「実行と判定の分離」が**実際に E-4 の偽failを消すか**を確かめる。
...
### 対策（`ARCHITECTURE.md` §5.5 の簡報がそのまま解になる）
...
| 主張 | 状態 |
|---|---|
| 実行と判定の分離で偽failが消える | **実証済み** |
| 裁定4分類が機能する | **実証済み**（5ケース） |
| 証拠なき指摘の破棄が機能する | **実証済み** |
| レビュア役はベンダー差し替え可能 | **実証済み**（codex/agy） |
| 簡報の埋め込みが判定独立性に必須 | **実証済み**（§4の落とし穴） |
